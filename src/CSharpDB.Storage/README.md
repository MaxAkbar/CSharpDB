...
That does not mean every shared auto-commit workload now coalesces equally. Hot auto-commit inserts still stay on the legacy serialized path so the engine does not reopen next-row-id and shared-catalog conflict costs on the common insert loop. The targeted `ConcurrentDurableWrite_W8_Batch250us` insert rerun in `concurrent-write-scenario-W8_Batch250us-20260411-142023.csv` still measured about `428 commits/sec` with `commitsPerFlush = 1.00`. Treat `UseDurableGroupCommit(...)` as a measured knob for overlapping explicit `WriteTransaction` commits plus shared auto-commit non-insert contention, not as a guaranteed throughput switch for hot single-row insert loops.
