...
internal sealed record MySqlWorkerDependencies
{
    internal static MySqlWorkerDependencies Default { get; } = new();
    ...
    internal Func<string, IMigrationSourceInspector> CreateInspector { get; init; } =
        static connectionString =>
            new MySqlMigrationSourceInspector(connectionString);
...
{
    internal const string Protocol = "csharpdb-mysql-worker/v1";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (inspector is null ||
                inspector.SourceKind != MigrationSourceKind.MySql)
            {
                return await FailAsync(
                    error,
                    ExitInternalFailure,
        ...
        try
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (catalog is null ||
                catalog.Source.Kind != MigrationSourceKind.MySql ||
                !string.Equals(
