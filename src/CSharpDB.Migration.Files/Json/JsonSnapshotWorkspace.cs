...
/// <summary>
/// Exclusively reserves and marks one private JSON snapshot directory beneath
/// a caller-controlled parent. Cleanup is bounded to registered immediate
/// children and is allowed only while the random ownership marker still
/// matches.
/// </summary>
internal sealed class JsonSnapshotWorkspace : IAsyncDisposable
{
    private const string WorkspacePrefix = "csharpdb-json-";
    private const string OwnershipFileName = ".csharpdb-json-owner";
    ...
    internal JsonSnapshotWorkspace(string rootDirectory)
    {
        ...
        throw new IOException(
            $"Could not create an exclusively owned JSON snapshot workspace under '{RootDirectory}'.",
            finalFailure);
    }
    ...
    internal string GetImmediateChildPath(string fileName)
    {
            ...
            fileName is "." or "..")
        {
            throw new ArgumentException(
                "A JSON snapshot file name must identify one immediate workspace child.",
                nameof(fileName));
        }
    ...
    public ValueTask DisposeAsync()
    {
        ...
        // Cleanup is deliberately non-cancelable. The parent must remain
        // caller-controlled and stable for the workspace lifetime.
        try
        {
    ...
    private void VerifyOwnedDirectory()
    {
        ...
        {
            throw new IOException(
                "The JSON snapshot workspace directory has changed.");
        }
    }
    ...
    private void DeleteOwnedDirectoryContents()
    {
                 ...
                 Directory.EnumerateFileSystemEntries(DirectoryPath))
        {
            ...
            {
                throw new IOException(
                    "The JSON snapshot workspace contains an unowned child.");
            }
            ...
            {
                throw new IOException(
                    "The JSON snapshot workspace contains an unsafe or unexpected child.");
            }
        }
    ...
    private static void ValidateRootDirectory(string rootDirectory)
    {
        ...
        {
            throw new IOException(
                "The JSON snapshot workspace parent must be a real, caller-controlled directory.");
        }
    }
    ...
    {
        FileAttributes pathAttributes = File.GetAttributes(markerPath);
        if ((pathAttributes &
            (FileAttributes.Directory |
             FileAttributes.ReparsePoint |
             FileAttributes.Device)) != 0)
        {
            throw new IOException(
                "The JSON snapshot workspace ownership marker has changed.");
        }
        ...
        FileAttributes handleAttributes = File.GetAttributes(marker.SafeFileHandle);
        if ((handleAttributes &
            (FileAttributes.Directory |
             FileAttributes.ReparsePoint |
             FileAttributes.Device)) != 0 ||
            marker.Length != expectedToken.Length)
        {
            throw new IOException(
                "The JSON snapshot workspace ownership marker has changed.");
        }
        ...
        {
            throw new IOException(
                "The JSON snapshot workspace ownership token is invalid.");
        }

        marker.ReadExactly(actual);
        bool matches = actual.SequenceEqual(expectedToken);
    ...
    private static void CreatePrivateDirectoryExclusive(string path)
    {
        if (OperatingSystem.IsWindows())
        {
            ...
            {
                throw new IOException(
                    "The JSON snapshot workspace candidate already exists.",
                    nativeError);
            }
            if (error == 5)
            {
                throw new UnauthorizedAccessException(
                    "Access to the JSON snapshot workspace parent was denied.",
                    nativeError);
            }

            throw new IOException(
                "The JSON snapshot workspace could not be created.",
                nativeError);
        }
