...
internal static class JsonSnapshotPackageFile
{
    ...
    internal static FileStream OpenReadNoFollow(
        string fullPath,
        int bufferSize)
    {
        ...
        if (fullPath.Contains('\0'))
        {
            throw new ArgumentException(
                "The package path cannot contain a null character.",
                nameof(fullPath));
        }
        if (!Path.IsPathFullyQualified(fullPath))
        {
            throw new ArgumentException(
                "A fully qualified package path is required.",
                nameof(fullPath));
        }
        if (bufferSize <= 0)
        {
        ...
        try
        {
            ...
            if (!OperatingSystem.IsWindows() &&
                SystemNativeFcntlSetIsNonBlocking(
                    handle,
                    isNonBlocking: 0) != 0)
            {
                throw CreateIoException(
                    "The JSON package file could not be prepared for reading.",
    ...
    private static SafeFileHandle OpenUnix(string fullPath)
    {
        ...
        try
        {
            // SafeFileHandle treats zero as invalid. Move a valid descriptor
            // when standard input happened to be closed.
            if (descriptor == 0)
            {
                ...
                {
                    throw CreateIoException(
                        "The JSON package file could not be opened.",
                        duplicateError);
                }
            ...
            SafeFileHandle handle =
                new(new IntPtr(descriptor), ownsHandle: true);
            descriptor = -1;
            return handle;
        }
        finally
    ...
    private static int GetUnixOpenFlags()
    {
        ...
        throw new PlatformNotSupportedException(
            "Secure JSON package file opening is not implemented for this operating system.");
    }
    ...
    private static void ValidateRegularFile(SafeFileHandle handle)
    {
        ...
        catch (JsonSnapshotPackageException)
        {
            throw;
        }
    ...
    private static JsonSnapshotPackageException
        ...
        new(
            ...
            "The JSON package path must identify a regular file and cannot be a link or special file.");

    private static JsonSnapshotPackageException
        CreateUnsafePathException(Exception innerException) =>
        new(
            JsonSnapshotPackageRules.UnsafePath,
            "The JSON package path must identify a regular file and cannot be a link or special file.",
            innerException);
