...
/// </summary>
internal static class CsvSnapshotPackageCanonicalJson
{
    ...
    private static void WriteElement(ArrayBufferWriter<byte> writer, JsonElement element)
    {
        switch (element.ValueKind)
        {
            ...
            case JsonValueKind.String:
                ...
                try
                {
                    value = element.GetString() ?? throw new InvalidDataException(
                        "The CSV package manifest contains an invalid JSON string.");
                }
                catch (InvalidOperationException exception)
                {
                    throw new InvalidDataException(
                        "The CSV package manifest contains invalid UTF-16 text.",
                        exception);
                }
            ...
            default:
                throw new InvalidDataException(
                    "The CSV package manifest contains an unsupported JSON token.");
        }
    }
    ...
    private static void WriteString(ArrayBufferWriter<byte> writer, string value)
    {
        WriteByte(writer, (byte)'"');
        int segmentStart = 0;
        Span<char> unicodeEscape = stackalloc char[6] { '\\', 'u', '0', '0', '0', '0' };
    ...
    private static char Hex(int value) => "0123456789abcdef"[value & 0xF];

    private static void WriteUtf8(ArrayBufferWriter<byte> writer, ReadOnlySpan<char> value)
    {
        if (value.IsEmpty)
            return;
        try
    ...
    private static void WriteAscii(ArrayBufferWriter<byte> writer, ReadOnlySpan<char> value)
    {
        ...
        for (int index = 0; index < value.Length; index++)
        {
            char character = value[index];
            if (character > 0x7F)
                throw new InvalidDataException("The canonical JSON token is not ASCII.");
