...
public sealed class CsvStreamingReader : IAsyncDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            EncodingResolution resolution = ResolveEncoding(prefix.AsSpan(0, prefixLength), settings);
    ...
    private static EncodingResolution ResolveEncoding(
        ReadOnlySpan<byte> prefix,
        CsvReaderSettings settings)
    {
        if (settings.DetectEncodingFromByteOrderMarks)
        {
            if (prefix.StartsWith(new byte[] { 0x00, 0x00, 0xFE, 0xFF }))
                return new EncodingResolution(new UTF32Encoding(true, false, true), 4);
            if (prefix.StartsWith(new byte[] { 0xFF, 0xFE, 0x00, 0x00 }))
                return new EncodingResolution(new UTF32Encoding(false, false, true), 4);
            if (prefix.StartsWith(new byte[] { 0xEF, 0xBB, 0xBF }))
                return new EncodingResolution(new UTF8Encoding(false, true), 3);
            if (prefix.StartsWith(new byte[] { 0xFE, 0xFF }))
                return new EncodingResolution(new UnicodeEncoding(true, false, true), 2);
            if (prefix.StartsWith(new byte[] { 0xFF, 0xFE }))
                return new EncodingResolution(new UnicodeEncoding(false, false, true), 2);
        }

        return new EncodingResolution(settings.Encoding, 0);
    }
    ...
    private sealed record EncodingResolution(Encoding Encoding, int PreambleLength);

    private sealed class CsvMalformedDataException : Exception
    {
    }

    private sealed record CsvReaderSettings(
        ...
        bool LeaveOpen)
    {
        public static CsvReaderSettings Create(CsvReaderOptions options)
        {
            ...
            var encoding = (Encoding)options.Encoding.Clone();
