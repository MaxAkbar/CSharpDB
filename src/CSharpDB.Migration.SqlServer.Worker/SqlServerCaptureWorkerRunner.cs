...
internal static class SqlServerCaptureWorkerRunner
{
    ...
    internal static bool IsCaptureProtocol(IReadOnlyList<string> args) =>
        args.Count >= 2 &&
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (SqlServerRetainedCaptureLimitException)
        {
            return await FailAsync(
                error,
                ExitLimitExceeded,
        ...
        try
        {
            ...
            if (StrictUtf8.GetByteCount(json) > MaxResultBytes)
            {
                return await FailAsync(
                    error,
                    ExitInternalFailure,
        ...
        out CaptureResult response)
    {
        ...
        if (result?.Manifest is null ||
            ...
            !string.Equals(
                ...
                StringComparison.Ordinal) ||
            result.Manifest.SourceKind != MigrationSourceKind.SqlServer ||
            !IsPackageDigest(result.PackageDigest) ||
            !IsLowerSha256(result.Manifest.CatalogDigest) ||
            string.IsNullOrWhiteSpace(result.Manifest.SnapshotIdentity) ||
            result.Manifest.SnapshotIdentity.Length > 1_024)
        {
            return false;
        }
        ...
        response = new CaptureResult
        {
            ...
            CatalogDigest = result.Manifest.CatalogDigest,
            SnapshotIdentity = result.Manifest.SnapshotIdentity,
            PackageBytes = package.Length,
            TableCount = result.Manifest.Tables.Count,
            RowCount = rowCount,
        };
        return true;
    }
        ...
        out int tableTimeoutSeconds)
    {
        ...
        if (args.Count != 12 ||
            ...
            !TryValidateOutputPath(args[7], out string? safeOutputPath) ||
            !string.Equals(
                args[8],
                "--max-source-bytes",
                StringComparison.Ordinal) ||
            ...
            parsedMaximum <= 0 ||
            parsedMaximum > HardMaxPackageBytes ||
            !string.Equals(
                args[10],
        ...
        tableTimeoutSeconds = parsedTableTimeout;
        return true;
    }
        ...
        try
        {
            ...
            StringComparison pathComparison =
                ...
                OperatingSystem.IsMacOS()
                    ...
                    : StringComparison.Ordinal;
            if (!string.Equals(value, fullPath, pathComparison) ||
                !string.Equals(
            ...
            if (string.IsNullOrEmpty(parent) ||
                ...
                !TryValidateWorkspaceName(Path.GetFileName(parent)))
            {
                return false;
            }
    ...
    private static bool TryValidateWorkspaceName(string name)
    {
        if (!name.StartsWith(WorkspacePrefix, StringComparison.Ordinal))
            return false;
        string suffix = name[WorkspacePrefix.Length..];
    ...
    private static bool IsPackageDigest(string? value) =>
        value is { Length: 71 } &&
