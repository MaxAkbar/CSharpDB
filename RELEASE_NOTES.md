...
- Added a sequential local durable-write release gate for an idle fixed-SSD Windows machine after repeated GitHub-hosted runs demonstrated non-qualifying transient disk variance. The two balanced paired passes cover the ten file-backed, hybrid durable, and direct-client SQL/collection single and batch write rows, pin both engine commits, force durable mode, and retain hash-verified raw evidence outside the repository. The canonical `durable-v2` policy fails closed on invalid evidence or inconsistent blocking evidence, enforces throughput and P95 limits, and retains P99 as diagnostic evidence; a release-blocking P99 claim requires a separately designed longer experiment. Only that canonical policy can record the matching-commit, policy-bound status that the publishing workflow requires, and the status creator must match the configured attestor. GitHub continues to run two clean functional qualification passes on Windows, Linux, and macOS plus blocking paired comparisons for the other 18 stable master-table rows; broader performance suites remain manual or report-only diagnostics.
