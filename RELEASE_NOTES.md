...
- Added two clean, balanced paired previous-release master-table qualifications that run a hash-recorded candidate harness against both engines on the same runner. With three pairs per order, each pass alternates six adjacent pairs and records six samples per revision; pass two reverses the starting order. The gate evaluates paired effects separately in both order strata, requires a strict stable majority in each, and fails closed on malformed, unstable, or order-sensitive evidence. A stable candidate fails above the 15% throughput limit or when P99 exceeds both 25% and 0.05 ms. Broader suites remain available through the manual comparison runner, while the existing scheduled guardrail workflow provides report-only diagnostics without multiplying the release gate.
