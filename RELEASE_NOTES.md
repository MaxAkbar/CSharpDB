...
- Added two clean previous-release performance qualifications that run a hash-verified candidate harness against both engines. Every suite uses adjacent, alternating previous-candidate and candidate-previous pairs; each isolated sample receives the same unrecorded warmup, and each order stratum must independently provide strict stable majorities for throughput and P99. Regression requires both order strata to cross the same 15% throughput or dual 25% plus 0.05 ms P99 gate; malformed, unstable, and order-sensitive evidence fails closed. Every raw sample, pair manifest, and raw-evidence digest remains in runner-owned temporary storage rather than generated source artifacts.
