...
- Added two clean previous-release performance qualifications that run a hash-recorded candidate harness against both engines on the same runner. Each suite receives one unrecorded warmup and three recorded runs per revision; the gate recomputes each median from retained raw evidence, requires a strict stable majority within both revisions, and reverses revision order in pass two. A stable candidate fails above the 15% throughput limit or when P99 exceeds both 25% and 0.05 ms; malformed or unstable evidence fails closed. The higher-cost balanced-pair mode remains available for focused investigation rather than multiplying every release suite.
