internal sealed class PrefixedReplCommand : IReplCommand
{
    ...
    public PrefixedReplCommand(string name, string description, string usage, IReplCommand innerCommand, params string[] prefixArguments)
    {
        Name = name; Description = description; Usage = usage;
        _innerCommand = innerCommand; _prefixArguments = prefixArguments;
    }
    ...
    {
        return _innerCommand.ExecuteAsync(context, _prefixArguments.Concat(arguments).ToArray(), ct);
    }
}
