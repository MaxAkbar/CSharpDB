internal sealed class ReplHost
{
    ...
    {
        await _console.WriteBannerAsync("CSharpDB Virtual File System REPL", "Type 'help' for commands, 'exit' to quit");
        await _console.WriteInfoAsync($"Database file: {context.DatabasePath}");
        await _console.WriteInfoAsync($"Current directory: {context.CurrentDirectory}");
        await _console.WriteBlankLineAsync();

        while (!ct.IsCancellationRequested)
        {
            await _console.WritePromptAsync($"vfs:{context.CurrentDirectory}");
