...
| **Query** | No DISTINCT keyword |
| **Query** | No UNION / INTERSECT / EXCEPT |
| **RowId** | Generated by scanning all keys (O(n)) — no cached max counter |
| **Concurrency** | Single writer only (no multi-writer) |
| **Indexes** | Single-column indexes only (no composite indexes) |
| **Indexes** | Only integer-keyed index lookups (equality only, no range scans) |
