...
public sealed class JsonTypedSnapshotPackageProcessTests
{
#if DEBUG
    ...
    public async Task FreshProcessesWriteReadAndResumeExactTypedRows(
        string framingName)
    {
        ...
        await File.WriteAllTextAsync(
            ...
            Frame(
                framing,
                """{"id":"1","amount":"1.000000000000000001","binary":"AQI=","name":"alpha"}""",
                """{"id":"2","amount":"2.000000000000000002","binary":"AwQ=","name":"bravo"}""",
                """{"id":"3","amount":"3.000000000000000003","binary":"BQY=","name":"charlie"}""",
                """{"id":"4","amount":"4.000000000000000004","binary":"Bwg=","name":"delta"}""",
                """{"id":"5","amount":"5.000000000000000005","binary":"CQo=","name":"echo"}""",
                """{"id":"6","amount":"6.000000000000000006","binary":"Cww=","name":"foxtrot"}""",
                """{"id":"7","amount":"7.000000000000000007","binary":"DQ4=","name":"golf"}"""),
            s_strictUtf8,
            Cancellation);
    ...
    private static void AssertCanonicalHexDigest(
        string digest)
    {
    ...
    private static void AssertLowerHex(
            ...
            CancellationToken cancellationToken)
    {
        ...
        return JsonSerializer.Deserialize<
            ...
            ?? throw new InvalidDataException(
                "The typed JSON package process result is empty.");
    }
            ...
            CancellationToken cancellationToken)
    {
        ...
        if (!process.Start())
        {
            throw new InvalidOperationException(
                "Failed to start the typed JSON package process harness.");
        }
                ...
                .IsCancellationRequested)
        {
            ...
            throw new TimeoutException(
                "The typed JSON package process harness exceeded its 30-second timeout.");
        }
        catch (OperationCanceledException)
        {
            if (!process.HasExited)
    ...
    private sealed class TemporaryDirectory : IDisposable
    {
        internal TemporaryDirectory()
        {
            Root = Path.GetFullPath(
                Path.Combine(
                    Path.GetTempPath(),
                    "csharpdb-json-typed-package-process",
